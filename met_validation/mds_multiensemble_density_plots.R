...
for (d in debias_seq){
  for (e in ens_seq){
    tem = temp.df[paste0("tair",d, ".", e)]
    colnames(tem) = "v1"
    p1 = p1 + geom_density(data = tem, aes(x=v1, y=..density.., color='Debiased Ensemble Members'), size=0.5, fill="lightblue", alpha=0.5) # + xlim(240,315) + ylim(0, 0.05)
  }
}
p1 = p1 + geom_density(data = nldas, aes(x=air_temperature, y=..density.., color = 'NLDAS Raw'), size=1) 
p1 = p1 + geom_density(aes(x=air_temperature, y=..density..), colour="black", size=1.5) 
...
for (d in debias_seq){
  for (e in ens_seq){
    tem = temp.df[paste0("tair",d, ".", e)]
    colnames(tem) = "v1"
    p2 = p2 + geom_density(data = tem, aes(x=v1, y=..density.., color='Debiased Ensemble Members'), size=0.5, fill="lightblue", alpha=0.5) # + xlim(240,315) + ylim(0, 0.05)
  }
}
p2 = p2 + geom_density(data = nldas, aes(x=nldas_airtempmax, y=..density.., color = 'NLDAS Raw'), size=1)
p2 = p2 + geom_density(aes(x=air_temperature, y=..density..), colour="black", size=1) + theme(legend.position="none")
...
for (d in debias_seq){
  for (e in ens_seq){
    tem = temp.df[paste0("tair",d, ".", e)]
    colnames(tem) = "v1"
    p3 = p3 + geom_density(data = tem, aes(x=v1, y=..density.., color='Debiased Ensemble Members'), size=0.5, fill="lightblue", alpha=0.5) # + xlim(240,315) + ylim(0, 0.05)
  }
}
p3 = p3 + geom_density(data = nldas, aes(x=nldas_airtempmin, y=..density.., color = 'NLDAS Raw'), size=1) 
p3 = p3 + geom_density(aes(x=air_temperature, y=..density..), colour="black", size=1) + theme(legend.position="none")
