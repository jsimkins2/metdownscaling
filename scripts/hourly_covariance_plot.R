...
for (i in seq_len(length(ind.list))){
  ...
  ens.df$month = months
  ens.df$year = years
  grouped = group_by(ens.df,hour_of_day)
  ...
  ens.res$hour_of_day1 = NULL
  ens.res$hour_of_day = NULL
  assign(name_enscor[i], cor(ens.res))
  assign(name_ensdf[i], ens.res)
}


png("~/Documents/metdownscaling/plots/validate/hourly_met_corrplots.png", width = 4, height = 16, units = "in",res = 200)
...
title("Willow Creek Aggregated Hourly Correlations")

# Plot the NLDAS Hourly Correlations
...
corrplot(nldas.cor, p.mat = res1$p, method = 'color', number.cex = .7, type = 'lower',
         ...
         # Combine with significance
         sig.level = 0.05, insig = "blank")
title("Raw NLDAS Hourly Correlations")
...
corrplot(ens.cor.mean, p.mat = res1$p, method = 'color', number.cex = .7, type = 'lower',
         ...
         # Combine with significance
         sig.level = 0.05, insig = "blank")
title("Mean Ensemble Aggregated Hourly Correlations")
...
corrplot(ens.cor.pos.sd, p.mat = res1$p, method = 'color', number.cex = .7, type = 'lower',
         ...
         # Combine with significance
         sig.level = 0.05, insig = "blank")
title("+1 SD Ensemble Aggregated Hourly Correlations")
