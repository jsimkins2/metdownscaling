# resample to daily resolution and calculate correlations
# need to double check the resample to a daily res
...
grouped = group_by(wcr.df,day)
wcr.res = summarise_each(grouped,list(mean),air_temperature,surface_downwelling_shortwave_flux_in_air,
...
#wcr.res = append(wcr.res, summarise_each(grouped,list(sum), precipitation_flux))

wcr.res = data.frame(wcr.res)
wcr.res$day = NULL
wcr.cor = cor(wcr.res)
...
for (d in debias_seq){
  for (e in ens_seq){
    ...
    ens.df$month = months
    ens.df$year = years
    grouped = group_by(ens.df,day)
    ens.res = summarise_each(grouped,list(mean),air_temperature,surface_downwelling_shortwave_flux_in_air,
                             specific_humidity, surface_downwelling_longwave_flux_in_air, air_pressure, wind_speed, precipitation_flux)
    
    ens.res = data.frame(ens.res)
    ens.res$year.1 = NULL
    ens.res$year = NULL
    ens.cor = cor(ens.res)
...
for (i in seq_len(length(ind.list))){
  ...
  ens.res$year.1 = NULL
  ens.res$year = NULL
  assign(name_enscor[i], cor(ens.res))
  assign(name_ensdf[i], ens.res)
}


png("~/Documents/metdownscaling/plots/validate/daily_met_corrplots.png", width = 4, height = 16, units = "in",res = 200)
